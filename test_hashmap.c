...
{
    struct hashmap_insert_ret ret = hashmap_insert(h, key);
    assert(ret.ok == true);

    struct bucket *b = (struct bucket *)ret.pair.userdata;
    b->value = value;
...
{
    struct hashmap_iter *iter = hashmap_find(h, key);
    assert(iter != hashmap_end(h));

    struct hashmap_pair pair = hashmap_iter_deref(iter);
...
int main(void)
{
    ...
    struct hashmap *h = hashmap_new(sizeof(struct bucket));
    ...
    struct hashmap_iter *iter = hashmap_begin(h);
    assert(iter == hashmap_end(h));
    ...
    struct hashmap_insert_ret ret = hashmap_insert(h, "bacteria");
    assert(ret.ok == false);
    struct bucket *b = (struct bucket *)ret.pair.userdata;
    assert(b->value == 10);
    b->value = 1;
    ...
    struct hashmap_pair pair = hashmap_iter_deref(hashmap_end(h));
    ...
    float d = fabsf(0.f - hashmap_load_factor(NULL));
    assert(d <= FLT_MIN);
