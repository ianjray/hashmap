...
struct hashmap *hashmap_new(size_t element_size)
{
    ...
    h->buckets = 0;
    h->list = list_new(element_size, node_destructor);
    h->map = NULL;
...
{
    return (struct hashmap_iter *)list_iter_inc((struct list_iter *)iter);
}

struct hashmap_pair hashmap_iter_deref(struct hashmap_iter *iter)
{
    struct hashmap_node *node = list_iter_deref((struct list_iter *)iter);
...
struct hashmap_iter *hashmap_find(struct hashmap *h, const char *key)
{
    ...
    for (iter = h->map[bucket]; iter; iter = list_iter_inc(iter)) {
        struct hashmap_node *node = list_iter_deref((struct list_iter *)iter);
...
struct hashmap_insert_ret hashmap_insert(struct hashmap *h, const char *key)
{
    ...
    if (iter != list_end(h->list)) {
        ...
        node = (struct hashmap_node *)list_insert(h->list, iter);
        node->hash = hash;
        node->bucket = bucket;
        node->key = strdup(key);

        if (h->map[bucket] == NULL) {
            h->map[bucket] = list_iter_dec(iter);
        }
...
void hashmap_erase(struct hashmap *h, struct hashmap_iter *iter)
{
    if (!h) {
        ...
        if (h->map[bucket] == (struct list_iter *)iter) {
            struct list_iter *next = list_iter_inc((struct list_iter *)iter);

            if (next != list_end(h->list) && ((struct hashmap_node *)list_iter_deref((struct list_iter *)next))->bucket == bucket) {
                h->map[bucket] = next;
        ...
        list_erase(h->list, (struct list_iter *)iter);
    }
}
...
void hashmap_rehash(struct hashmap *h, size_t n)
{
    ...
    for (iter = list_begin(h->list); iter != list_end(h->list); ) {
        struct list_iter *next = list_iter_inc(iter);
        struct hashmap_node *node = (struct hashmap_node *)list_iter_deref(iter);

        if ((struct list_iter *)hashmap_find(h, node->key) == list_end(h->list)) {
            node->bucket = hashmap_bucket(h, node->key);

            if (h->map[node->bucket]) {
                list_splice(h->list, h->map[node->bucket], iter);
            }
