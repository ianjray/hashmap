...
static int readdict(const char *filename, struct hashmap *h)
{
    ...
    while (readline(fp, buffer, sizeof(buffer)) == 0) {
        if (isupper(buffer[0])) {
            ...
            struct hashmap_insert_result ret = hashmap_insert(h, buffer);
            assert(ret.inserted);
        }
    }
...
int main(void)
{
    ...
    if (r == 0) {
        const struct hashmap_iter *cit;
        printf("size %zu\n", hashmap_size(h));
        printf("buckets %zu\n", hashmap_bucket_count(h));
        printf("load factor %f\n", (double)hashmap_load_factor(h));

        cit = hashmap_find(h, "hasten");
        if (cit != hashmap_cend(h)) {
            const struct hashmap_pair *pair = hashmap_at_const(cit);
            printf("found {%s}\n", pair->key);
        }
    }
